...
// A project-local .env is the primary configuration. Clear an inherited stale
// value first: Node intentionally does not let loadEnvFile overwrite it.
const inheritedDataDir = process.env.NOVEL_DATA_DIR;
delete process.env.NOVEL_DATA_DIR;
try {
  process.loadEnvFile(envFile);
} catch (err) {
  if (err?.code !== 'ENOENT') throw err;
}
if (!process.env.NOVEL_DATA_DIR && inheritedDataDir) {
  process.env.NOVEL_DATA_DIR = inheritedDataDir;
}

// Set NOVEL_DATA_DIR to make this reader use a shared Translator data folder.
// Keep the local data/ fallback so a standalone clone remains usable.
const defaultDataDir = path.join(__dirname, '..', 'data');
const configuredDataDir = process.env.NOVEL_DATA_DIR?.trim();
export const DATA_DIR = path.resolve(configuredDataDir || defaultDataDir);
const COVER_TYPES = [
  ['jpg', 'image/jpeg'],
  ['png', 'image/png'],
  ['webp', 'image/webp'],
...
/** The optional cover and a version token, never a filesystem path for a view. */
export async function coverInfo(novel) {
  const dir = resolveWithin(novel);
  if (!dir) return null;
  for (const [extension, mediaType] of COVER_TYPES) {
    const file = path.join(dir, `cover.${extension}`);
    try {
      const stat = await fs.stat(file);
      if (stat.isFile()) return { file, mediaType, version: Math.trunc(stat.mtimeMs) };
    } catch {
      // An absent cover is normal; try the next supported format.
    }
  }
  return null;
}

/** Latest write time of the chapter text the reader would actually display. */
export async function latestReadableUpdate(novel, refs) {
  const dir = resolveWithin(novel, 'chapters');
  if (!dir) return 0;

  const times = await mapLimit(refs, 64, async (ref) => {
    for (const name of ['final.txt', 'draft.txt']) {
      try {
        const stat = await fs.stat(path.join(dir, ref, name));
        if (stat.size) return stat.mtimeMs;
      } catch {
        // A chapter can be changed while its shelf entry is being calculated.
      }
    }
    return 0;
  });
...
export async function listNovels() {
  ...
  for (const e of entries) {
    ...
    const profile = await readJson(path.join(dir, '_novel.json'));
    if (!profile) continue;
    const refs = await chapterRefs(e.name);
    const cover = await coverInfo(e.name);
    let addedAt = 0;
    try {
      const stat = await fs.stat(dir);
      // birthtime means the folder was first added on Windows. ctime is the
      // closest portable fallback for filesystems without a creation time.
      addedAt = stat.birthtimeMs || stat.ctimeMs;
    } catch {
      // Keep the novel on the shelf even if it disappears mid-scan.
    }
    novels.push({
      name: e.name,
      profile,
      ...
      addedAt,
      updatedAt: await latestReadableUpdate(e.name, refs),
    });
  }
